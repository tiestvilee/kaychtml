...
class KaychTMLFormatterTest {
    ...
    companion object {
        ...
        fun data() = listOf(
            ...
            arrayOf(
                "Outputs a single tag with an id",
                html(id("the-id")),
                """<html id="the-id">
                |</html>""".trimMargin(),
                """<html id="the-id"></html>"""
            ...
            arrayOf(
                "Outputs a single tag with a class",
                html(cl("className")),
            ...
            arrayOf(
                "Outputs a single tag with multiple classes",
            ...
            arrayOf(
                "Outputs a single tag with lots of attributes",
                html(
                    id("the-id"),
                    cl("className"),
                    "data-test" attr "This is an \"attribute\" <but this isn't a tag>",
                    cl("otherClassName")
                ),
            ...
            arrayOf(
                "empty tags render in a special way",
            ...
            arrayOf(
                ...
                |︎      font-family: monospace;
                |︎      background-color: #eee;
                |︎  }</style></head><body><h1>no conflicts found</h1></body></html>""".trimMargin()
            ),

            arrayOf(
                "bigger test 2",
                biggerHtml(
                    listOf(Conflict(setOf(Scientist("con-A"), Scientist("con-B")), Scientist("auth-c"))),
                    listOf(Scientist("unknown-d"))
                ),
                """<!DOCTYPE html>
                |<html>
                |  <head>
                ...
                |︎      font-family: monospace;
                |︎      background-color: #eee;
                |︎  }</style></head><body><h1>Potential conflicts</h1><ul id="conflicts"><li class="conflict"><span class="author">auth-c</span>worked with <ul><li>con-A</li><li>con-B</li></ul></li></ul><h1>Couldn't find the following scientists</h1><ul id="missingScientists"><li>unknown-d</li></ul></body></html>""".trimMargin()
            )
                    ...
                    body(
                            ...
                            elements(
                                ...
                                ul(
                                    ...
                                    conflicts.map { conflict ->
                                        li(
                                            cl("conflict"),
                                            span(cl("author"), conflict.author.name.s),
                                            "worked with ".s,
                                            ul(
                        ...
                        if (scientists.isNotEmpty()) {
                            elements(
                                ...
                                ul(
                                    id("missingScientists"),
                                    scientists.map { scientist -> li(scientist.name.s) }.elements()
                                )
                            )
                        } else {
                            KNoop()
    ...
    @ParameterizedTest
    @MethodSource("data")
    fun `pretty formatting`(name: String, element: KTag, prettyResult: String, compactResult: String) {
        assertThat(element.toHtml(), equalTo(prettyResult))
    }

    @ParameterizedTest
    @MethodSource("data")
    fun `compact formatting`(name: String, element: KTag, prettyResult: String, compactResult: String) {
